...
struct ScaleThresholds {
    max_recall_drop_100_to_5000: f64,
    max_knn_mrr_drop_100_to_5000: f64,
}

#[derive(Debug, Serialize)]
struct ScaleEvalReport {
    suite: &'static str,
    encoder_model: String,
    query_panel_size: usize,
...
const SCALE_ALIAS_OPERATIONS: &[&str] = &[
    "spin",
    "rerun",
    "safety rail",
    "swapover",
...
const SCALE_ALIAS_MODIFIERS: &[&str] = &[
    "buffered",
    "persistent",
    "stepwise",
    "after-hours",
...
const SCALE_WORKSTREAM_ADJECTIVES: &[&str] = &[
    "amber",
    "atlas",
    "aurora",
    "brisk",
...
const SCALE_WORKSTREAM_NOUNS: &[&str] = &[
    "bridge",
    "compass",
    "corridor",
    "deck",
...
const SCALE_MISSION_DOMAINS: &[&str] = &[
    "billing",
    "catalog",
    "checkout",
    "compliance",
...
const SCALE_MISSION_THEATERS: &[&str] = &[
    "program",
    "readiness lane",
    "reliability track",
    "operations stream",
...
const SCALE_TAG_GROUPS: &[(&str, &str, &str)] = &[
    ("retrieval", "operations", "design"),
    ("storage", "reliability", "workflow"),
    ("api", "compatibility", "governance"),
    ("security", "credentials", "guardrails"),
    ("kubernetes", "availability", "scheduling"),
    ("analytics", "history", "modeling"),
    ("observability", "incident-response", "noise-control"),
    ("queueing", "backpressure", "throughput"),
];
...
fn build_scale_corpus(target_note_count: usize) -> EvalCorpus {
    assert_eq!(
        target_note_count % 4,
        0,
        "scale corpora should be family aligned"
    );
    let family_count = target_note_count / 4;
    let mut notes = Vec::with_capacity(target_note_count);
    let mut queries = Vec::with_capacity(20);

    for family_index in 0..family_count {
        let family = build_scale_family(family_index);
        if family_index < 20 {
            queries.push(QueryCase {
                name: format!("family-{family_index:04}-alias"),
                query: family.query,
                expected_slugs: vec![family.core_slug.clone(), family.companion_slug.clone()],
                expected_linked_slug: Some(family.companion_slug.clone()),
            });
        }
        notes.extend(family.notes);
    }
...
fn build_scale_family(family_index: usize) -> ScaleFamily {
    let system = SCALE_SYSTEMS[(family_index * 7) % SCALE_SYSTEMS.len()];
    let operation_index = (family_index * 11) % SCALE_OPERATIONS.len();
    let operation = SCALE_OPERATIONS[operation_index];
    let alias_operation = SCALE_ALIAS_OPERATIONS[operation_index];
    let modifier = SCALE_MODIFIERS[(family_index * 3) % SCALE_MODIFIERS.len()];
    let alias_modifier = SCALE_ALIAS_MODIFIERS[(family_index * 5) % SCALE_ALIAS_MODIFIERS.len()];
    let tag_group = SCALE_TAG_GROUPS[(family_index * 7) % SCALE_TAG_GROUPS.len()];
    let distractor_system = SCALE_SYSTEMS[(family_index * 13 + 5) % SCALE_SYSTEMS.len()];
    let distractor_operation = SCALE_OPERATIONS[(family_index * 17 + 3) % SCALE_OPERATIONS.len()];
    let workstream = format!(
        "{} {}",
        SCALE_WORKSTREAM_ADJECTIVES[(family_index * 19) % SCALE_WORKSTREAM_ADJECTIVES.len()],
        SCALE_WORKSTREAM_NOUNS[(family_index * 23) % SCALE_WORKSTREAM_NOUNS.len()],
    );
    let workstream_key = format!(
        "{}-{}-{:04}",
        SCALE_WORKSTREAM_ADJECTIVES[(family_index * 19) % SCALE_WORKSTREAM_ADJECTIVES.len()],
        SCALE_WORKSTREAM_NOUNS[(family_index * 23) % SCALE_WORKSTREAM_NOUNS.len()],
        family_index
    );
    let cutover_window = SCALE_WINDOWS[(family_index * 29) % SCALE_WINDOWS.len()];
    let mission = if family_index < 20 {
        format!(
            "{} {} {}",
            SCALE_MISSION_DOMAINS[family_index % SCALE_MISSION_DOMAINS.len()],
            SCALE_MISSION_GOALS[family_index % SCALE_MISSION_GOALS.len()],
            SCALE_MISSION_THEATERS[family_index % SCALE_MISSION_THEATERS.len()],
        )
    } else {
        format!(
            "{} {} {}",
            SCALE_MISSION_DOMAINS[family_index % SCALE_MISSION_DOMAINS.len()],
            SCALE_MISSION_GOALS
                [(family_index / SCALE_MISSION_DOMAINS.len()) % SCALE_MISSION_GOALS.len()],
            SCALE_MISSION_THEATERS[(family_index
                / (SCALE_MISSION_DOMAINS.len() * SCALE_MISSION_GOALS.len()))
                % SCALE_MISSION_THEATERS.len()],
        )
    };

    let canonical = format!("{modifier} {system} {operation}");
    let alias = format!("{alias_modifier} {system} {alias_operation}");
    let family_slug = format!("family-{family_index:04}");
    let core_slug = format!("{family_slug}-core");
    let companion_slug = format!("{family_slug}-companion");

    let notes = vec![
        SeedNote {
            slug: core_slug.clone(),
            content: format!(
                "{canonical} is the label used by the {workstream} team for the {mission} under workstream key {workstream_key} when operators prepare a {cutover_window}."
            ),
            context: format!(
                "The same workstream also uses \"{alias}\" as shorthand when discussing the {mission}, recovery steps for the {cutover_window}, and the identifier {workstream_key} in runbooks."
            ),
            keywords: vec![
                canonical.clone(),
                alias.clone(),
                ...
                operation.to_string(),
                workstream.clone(),
                workstream_key.clone(),
                cutover_window.to_string(),
                mission.clone(),
            ],
            tags: vec![
                tag_group.0.to_string(),
                tag_group.1.to_string(),
                tag_group.2.to_string(),
            ],
            linked_to: vec![companion_slug.clone()],
        },
        SeedNote {
            slug: companion_slug.clone(),
            content: format!(
                "The {workstream} companion note uses the phrase \"{alias}\" for the operator checklist that supports {canonical} during the {cutover_window} for the {mission} under workstream key {workstream_key}."
            ),
            context: format!(
                "This companion memory explains the supporting workflow around {canonical}, preserves the alias phrasing for the same workstream, and repeats both {mission} and the identifier {workstream_key}."
            ),
            keywords: vec![
                alias.clone(),
                canonical.clone(),
            ...
            tags: vec![
                tag_group.0.to_string(),
                tag_group.1.to_string(),
                "linked-companion".to_string(),
            ],
            linked_to: vec![core_slug.clone()],
        },
        SeedNote {
            slug: format!("{family_slug}-distractor"),
            content: format!(
                "The {workstream} backlog also tracks {modifier} {distractor_system} {distractor_operation}, which shares tags with {canonical} but solves a different operational problem than the {mission} tracked under workstream key {workstream_key}."
            ),
            context: format!(
                "This distractor note intentionally shares some vocabulary with {canonical} and the same {cutover_window} while staying outside the main concept family."
            ),
            keywords: vec![
                distractor_system.to_string(),
                distractor_operation.to_string(),
                modifier.to_string(),
                cutover_window.to_string(),
                workstream.clone(),
                mission.clone(),
        ...
        SeedNote {
            slug: format!("{family_slug}-filler"),
            content: format!(
                "The {workstream} team keeps a separate housekeeping note about archival checklists and review cadence for the {mission} that does not define {canonical}, even though it still references workstream key {workstream_key}."
            ),
            context: format!(
                "This filler note gives the corpus extra load around the {cutover_window} without acting as a semantic synonym for {canonical}."
            ),
            keywords: vec![
                workstream.clone(),
                workstream_key.clone(),
    ...
    ScaleFamily {
        notes,
        query: format!(
            "How do we handle {alias} for the {mission} on workstream {workstream_key} with the {workstream} team before the {cutover_window}?"
        ),
        core_slug,
        companion_slug,
    }
...
async fn run_scale_eval(encoder_model: String, encoder: &EncoderClient) -> ScaleEvalReport {
    let mut rows = Vec::new();
    for note_count in [100, 1_000, 5_000] {
        let corpus = build_scale_corpus(note_count);
        rows.push(run_matrix_row(format!("scale@notes={note_count}"), &corpus, encoder, 10).await);
    }
    ...
    ScaleEvalReport {
        ...
        query_panel_size: 20,
        thresholds: ScaleThresholds {
            max_recall_drop_100_to_5000: 0.05,
            max_knn_mrr_drop_100_to_5000: 0.10,
        },
        rows,
    }
}
...
#[tokio::test]
#[ignore]
async fn real_encoder_scale_retrieval_eval() {
    if !evals_enabled("real_encoder_scale_retrieval_eval") {
        return;
    }

    let _guard = eval_lock().lock().await;
    let initialized = initialize_real_encoder();
    let report = run_scale_eval(initialized.model_id.clone(), &initialized.client).await;
    let pretty = serde_json::to_string_pretty(&report).expect("serialize scale report");
    println!("{pretty}");

    let baseline = report
        .rows
        .iter()
        .find(|row| row.note_count == 100)
        .expect("100-note baseline");
    let high_load = report
        .rows
        .iter()
        .find(|row| row.note_count == 5_000)
        .expect("5000-note row");

    let recall_drop = baseline.mean_recall - high_load.mean_recall;
    let knn_mrr_drop = baseline.mean_knn_reciprocal_rank - high_load.mean_knn_reciprocal_rank;

    assert!(
        recall_drop <= report.thresholds.max_recall_drop_100_to_5000,
        "{pretty}"
    );
    assert!(
        knn_mrr_drop <= report.thresholds.max_knn_mrr_drop_100_to_5000,
        "{pretty}"
    );
}
